...
When column resizing is enabled, double-click on a resize handle to auto-fit the column width to its content. On touch devices, double-tap the resize handle.
