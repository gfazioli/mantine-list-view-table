...
The `ListViewTable` component follows the Mantine-native responsive CSS approach. Props like `height`, `width`, `horizontalSpacing`, `verticalSpacing`, `headerTitleFontSize`, `headerTitleFontWeight`, `cellFontSize`, and `cellFontWeight` accept responsive objects keyed by breakpoint, generating CSS media queries for each value.

This means no JavaScript re-renders on resize — values are resolved purely via CSS `@media` rules.

```tsx
// Simple scalar value (works as before)
<ListViewTable height={400} />

// Responsive object per breakpoint
<ListViewTable
...
<Demo data={demos.responsiveProps} />
...
Control which columns are visible with the `hiddenColumns` prop.

<Demo data={demos.columnVisibility} />

### Column Visibility Toggle

Set `enableColumnVisibilityToggle` to allow users to toggle column visibility by right-clicking on the table header. On touch devices, long-press the header to open the same menu.

> The built-in menu prevents hiding the last visible column — when only one column remains, its menu item is automatically disabled. This safeguard only applies to the built-in toggle menu; programmatic control via `hiddenColumns` is unrestricted.

<Demo data={demos.columnVisibilityToggle} />
...
Enable `enableColumnReordering` to let users rearrange columns by dragging the grip handle on each header. The drag system runs on a single unified pointer-event path — **mouse, touch, and pen** all share the same code, so behavior is identical on desktop and mobile (including iOS Safari, where HTML5 drag-and-drop is unreliable).

A custom ghost element follows the pointer, the target column is highlighted via the `data-drag-over` attribute, and the new position commits on release. Press **`Escape`** during a drag to cancel without committing the move.

Use `onColumnReorder` to receive the `(fromIndex, toIndex)` callback for controlled mode, or omit it to let the component manage the column order internally.

<Demo data={demos.reordering} />
